...
int insert_into_sorted(int sourceArray[], int sourceLength, int destinationArray[], int newElement) {
    bool inserted = false;
    
    for (int i = 0; i < sourceLength; i++) {
        if (sourceArray[i] < newElement) {
            destinationArray[i] = sourceArray[i];
        } else if (sourceArray[i] >= newElement) {
            if (inserted == false) {
                destinationArray[i] = newElement;
                inserted = true;
                i -= 1;
            } else {
                destinationArray[i + 1] = sourceArray[i];
            }
        }
    }

    if (inserted == false) {
        destinationArray[sourceLength] = newElement;
    }
