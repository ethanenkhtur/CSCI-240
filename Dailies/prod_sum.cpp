...
int sum_of_rest_of_array(int [], int , int );
...
int prod_sum( int array1[], int array2[], int num_values) {
    if (num_values < 2) {
        return 0;
    } else {
        for (int i = 0; i < num_values; i++)
            array2[i] = array1[i] * sum_of_rest_of_array(array1, i, num_values);
        return 1;
    }
}

int sum_of_rest_of_array(int array[], int index, int num_values) {
    int sum = 0;
